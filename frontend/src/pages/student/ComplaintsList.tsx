...
const statusMap: Record<ComplaintStatus, { label: string, color: string, icon: any }> = {
  pending: { label: 'Pending', color: 'bg-slate-500/15 text-slate-500 border-slate-500/20 dark:text-slate-400', icon: Clock },
  in_progress: { label: 'In Progress', color: 'bg-blue-500/15 text-blue-500 border-blue-500/20 dark:text-blue-400', icon: MessageCircle },
  ...
  const filteredComplaints = React.useMemo(() => {
    ...
    if (searchTerm.trim() !== '') {
      ...
      result = result.filter(c => 
        ...
        (c.Category?.name && c.Category.name.toLowerCase().includes(searchLower)) ||
    ...
    <div className="space-y-8 animate-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">Track the status of your submitted issues or submit a new one</p>
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-slate-500 font-medium mt-4">Loading your complaints...</p>
        </div>
      ) : error ? (
        <Card className="border-red-200 dark:border-red-900 bg-red-500/5 p-6 text-center max-w-lg mx-auto">
          <AlertTriangle className="h-10 w-10 text-red-500 mx-auto mb-2" />
          <h3 className="font-bold text-slate-800 dark:text-slate-200">Error Occurred</h3>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">{error}</p>
          <Button onClick={fetchComplaintsAndCategories} className="mt-4" variant="outline">
            Try Again
          </Button>
        </Card>
      ) : (
        <>
            ...
            <div className="p-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-4 justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800">
                  {['all', 'pending', 'in_progress', 'resolved', 'appealed'].map((status) => (
                    <button
                      key={status}
              ...
              {filteredComplaints.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredComplaints.map((complaint) => {
                    const status = statusMap[complaint.status] || { label: complaint.status, color: 'bg-slate-500/10 text-slate-500', icon: Clock };
                    return (
                      <Link
                        key={complaint.id}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-6 ml-4">
                          <Badge className={cn("px-3 py-1 font-bold flex items-center gap-1 border", status.color)}>
                            <status.icon size={12} />
                            {status.label}
                          </Badge>
                          <ChevronRight size={20} className="text-slate-300 group-hover:text-slate-500 dark:group-hover:text-slate-400 transition-colors" />
                        </div>
                      </Link>
                    );
                  })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              { label: 'Pending', value: stats.pending, color: 'bg-slate-500', border: 'border-slate-500/20' },
              { label: 'In Progress', value: stats.in_progress, color: 'bg-blue-500', border: 'border-blue-500/20' },
              { label: 'Resolved', value: stats.resolved, color: 'bg-emerald-500', border: 'border-emerald-500/20' },
              { label: 'Appealed', value: stats.appealed, color: 'bg-orange-500', border: 'border-orange-500/20' },
            ].map(stat => (
              <Card key={stat.label} className={cn("border bg-white dark:bg-slate-900/50 shadow-sm", stat.border)}>
                <CardContent className="p-6 flex items-center gap-4">
                  <div className={cn("w-2.5 h-12 rounded-full", stat.color)} />
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{stat.label}</p>
                    <h4 className="text-3xl font-bold mt-1 text-slate-800 dark:text-slate-100">{stat.value}</h4>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <div className="fixed inset-0 bg-slate-955/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            <CardHeader className="flex flex-row items-start justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                <CardTitle className="text-xl font-bold text-slate-800 dark:text-white">Submit New Complaint</CardTitle>
                <CardDescription className="text-slate-500 dark:text-slate-400 mt-1">Please enter the details of the issue you are experiencing.</CardDescription>
              </div>
            ...
            <form onSubmit={handleSubmitComplaint}>
              <CardContent className="space-y-4 pt-6">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Complaint Category <span className="text-red-500">*</span></label>
                  <select
                    value={categoryId}
