...
interface BackendCategory { id: number; name: string; }
...
export default function OfficerDashboard() {
  ...
  // Fetch complaints + stats (category_id optional — backend already scopes to officer)
  useEffect(() => {
    setLoading(true);
    setError(null);
    ...
    Promise.all([
      ...
      .catch(err => setError(err.response?.data?.error || 'Failed to load officer dashboard data.'))
      .finally(() => setLoading(false));
  }, [selectedCategoryId]);
  ...
  const filtered = complaints.filter(c => {
    ...
    const matchDate = !hasValidDate || ((!from || complaintDate! >= from) && (!to || complaintDate! <= to));
      ...
      <div className="space-y-8 animate-in">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={e => { const v = e.target.value; setSelectedCategoryId(v === 'all' ? 'all' : parseInt(v, 10)); }}
              disabled={loadingCats}
              className="h-10 px-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-sm focus:ring-2 focus:ring-blue-600 outline-none text-slate-800 dark:text-slate-200 font-semibold disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Average Time to Resolve', value: stats?.avgResolutionTime ?? '0d',   color: 'text-blue-500',    bar: 'bg-blue-500'    },
            { label: 'Resolved on Time',      value: stats?.slaCompliance     ?? '100%', color: 'text-indigo-500',  bar: 'bg-indigo-500'  },
          ].map((s, i) => (
            <Card key={i} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  {loading ? <Skeleton className="h-8 w-16 mt-1" /> : <h3 className={cn('text-2xl font-bold mt-0.5', s.color)}>{s.value}</h3>}
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <Card className="border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden bg-white/50 dark:bg-slate-900/30 backdrop-blur-xl">
          <div className="p-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-4 justify-between">
            <div className="relative flex-1 max-w-md">
              ...
              <Input placeholder="Search complaints by ID or text..." className="pl-10 h-10 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200"
                value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                <Input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                  className="h-10 w-36 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200" aria-label="Filter from date" />
                <span className="text-xs text-slate-400">to</span>
                <Input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
                  className="h-10 w-36 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200" aria-label="Filter to date" />
                {(dateFrom || dateTo) && (
                  <Button variant="ghost" size="sm" onClick={() => { setDateFrom(''); setDateTo(''); }}
                    className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-200">Clear</Button>
                )}
              </div>
              {['all', 'pending', 'in_progress', 'resolved', 'appealed'].map(t => (
                <Button key={t} variant={filter === t ? 'default' : 'ghost'} size="sm" onClick={() => setFilter(t)}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {loading
                    ? [1,2,3,4].map(i => <tr key={i}><td colSpan={7} className="p-4"><Skeleton className="h-10 w-full" /></td></tr>)
                    : filtered.length === 0
                          ...
                          return (
                            <tr key={c.id} className={cn('hover:bg-slate-50 dark:hover:bg-slate-800/10 transition-colors group', justEscalated && 'bg-emerald-50/50 dark:bg-emerald-950/10')}>
                              ...
                              <td className="p-4 max-w-xs">
                                <p className="font-semibold text-slate-800 dark:text-slate-200 truncate group-hover:text-blue-600 transition-colors">{c.problem}</p>
