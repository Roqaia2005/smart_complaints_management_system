...
// faculty_id comes from the authenticated admin's token (set by authenticate middleware)
...
exports.createStudentController = async (req, res) => {
    try {
        ...
        if (!facultyId) {
            return res.status(400).json({ success: false, error: 'faculty_id is required' });
        }
...
exports.importStudentsPreviewController = async (req, res) => {
    try {
        ...
        const result = await adminService.importStudentsCsvService(req.file.path, facultyId);
...
exports.importOfficersPreviewController = async (req, res) => {
    try {
        ...
        const result = await adminService.importOfficersCsvService(req.file.path, facultyId);
        cleanupFile(req.file.path);
...
exports.importManagersPreviewController = async (req, res) => {
    try {
        ...
        const result = await adminService.importManagersCsvService(req.file.path, facultyId);
        cleanupFile(req.file.path);
...
// =========================================================
// USERS (general management — list / update / soft-delete only;
// creation now happens exclusively via the Students/Officers/Managers
// endpoints above)
// =========================================================
...
exports.deleteUser = (req, res) => {
    adminService.softDeleteUser(req.params.id)
        .then(() => res.status(200).json({ success: true }))
        .catch(err => res.status(500).json({ success: false, error: err.message }));
};
