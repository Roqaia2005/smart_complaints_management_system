...
// Key: import_id, Value: { type, facultyId, role, validRows, createdAt }
// =========================================================
...
// =========================================================
// Shared helpers (تعديل اللوجيك ليتوافق مع الـ Schema الفعلي)
// =========================================================
const isValidEmail = (email) => {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
};

// الاستعلام النصي المباشر الآمن لمنع أخطاء الـ Model Undefined والنوع في Postgres
      ...
      keywordList.forEach((kw) => {
        relationPromises.push(
          CategoryKeywords.create({
            ...
            keyword: kw,
          }).catch((err) => console.error("Failed to save keyword:", err.message)),
        );
      });
    }
...
exports.createUserService = async (data, facultyId) => {
  ...
  if (!role || !full_name || !email || !password) {
    throw new Error("role, full_name, email, and password are required fields.");
  }
  ...
  if (role === ROLES.STUDENT) {
    if (!student_number) throw new Error("student_number is required for creating a student.");
    
    // الحل السحري للأيرور: تحويل البحث لـ String صريح ليطابق نوع العمود VARCHAR بالداتا بيز
    ...
    const student = await Student.create({
      student_number: String(student_number), // حفظه كـ String
      full_name,
      email,
    ...
    if (validCategories.length !== category_ids.length) {
      throw new Error("One or more category_ids are invalid or belong to a different faculty.");
    }
    ...
    if (!manager_title || !manager_title.trim()) {
      throw new Error('manager_title is required for managers, e.g. "Academic Affairs Manager"');
    }
...
exports.importUsersCsvService = async (filePath, facultyId, targetRole) => {
  ...
  if (validation.invalid_structure) {
    throw new Error(`CSV layout is missing required headers for ${targetRole}: ${validation.missing_columns.join(", ")}`);
  }

  const existingUsers = await User.findAll({ attributes: ["email"] });
  const existingStudents = await Student.findAll({ attributes: ["email", "student_number"] });
  
  const globalEmails = new Set([...existingUsers.map(u => u.email), ...existingStudents.map(s => s.email)]);
  const globalStudentNumbers = new Set(existingStudents.map(s => s.student_number));
  const seenNumbersInFile = new Set();

  let validCategoryIds = new Set();
  if (targetRole === ROLES.OFFICER) {
    const categories = await Category.findAll({ where: { faculty_id: numericFacultyId }, attributes: ["id"] });
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    let cleanRowData = { full_name, email, password, role: targetRole, faculty_id: numericFacultyId };
    ...
    if (targetRole === ROLES.STUDENT) {
      ...
      if (student_number && (globalStudentNumbers.has(student_number) || seenNumbersInFile.has(student_number))) {
        errors.push("duplicate student identifier number matched");
      }
      seenNumbersInFile.add(student_number);
      
      cleanRowData.student_number = String(student_number); // ضمان تحويل الـ CSV لـ String
      cleanRowData.department = (row.department || "").trim() || null;
      cleanRowData.academic_year = row.academic_year ? parseInt(row.academic_year, 10) : null;
      ...
      if (!rawCategoryIds) {
        ...
        const parsedIds = rawCategoryIds.split(";").map(c => parseInt(c.trim(), 10)).filter(n => !isNaN(n));
        const invalidIds = parsedIds.filter(id => !validCategoryIds.has(id));
        if (invalidIds.length > 0) {
          errors.push(`unauthorized or out-of-bounds department categories: ${invalidIds.join(", ")}`);
        }
      ...
      if (!manager_title) errors.push("missing required operational management title");
  ...
  return {
    import_id: storeImportSession("unified_user", numericFacultyId, targetRole, validRows),
    preview: {
...
exports.confirmImportUsersService = async (importId) => {
  ...
  if (!session || session.type !== "unified_user") {
    throw new Error("Import payload references an invalid or expired caching context.");
  }
  ...
  try {
    ...
    for (const rowData of validRows) {
      ...
      if (role === ROLES.STUDENT) {
        const student = await Student.create({
          student_number: String(meta.student_number), // حفظه كـ String صريح
...
exports.createNewRegulation = (data) => {
  return Regulation.create({
    ...
    return axios
      ...
      .then(() => regulation)
      .catch((err) => {
        console.error(`Python sync failed for regulation ${regulation.id}:`, err.message);
        return regulation;
      });
  });
};
